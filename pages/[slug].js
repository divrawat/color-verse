...
export default function ShadesPage() {
    ...
    useEffect(() => {
        ...
        // Handle "shades-of-blue" format
        const colorKey = slug.replace('shades-of-', '');
        const data = COLOR_GROUPS[colorKey];
    ...
    if (!colorData) {
        return (
            <div className="text-center py-20">
                <h1 className="text-4xl font-bold text-gray-900 mb-4">Color Not Found</h1>
                <p className="text-gray-600 mb-8">Sorry, we couldn't find the shades for this color.</p>
                <Link href="/color-shades" className="px-6 py-3 bg-gray-900 text-white rounded-full font-bold">
                    Back to Directory
                </Link>
            </div>
        );
    ...
    const tints = chroma.scale(['#ffffff', colorData.hex]).mode('lch').colors(22).slice(1, 21).reverse();
    const shades = chroma.scale([colorData.hex, '#000000']).mode('lch').colors(22).slice(1, 21);
    ...
    return (
        <>
            <SEO
                ...
                canonicalUrl={`/${router.query.slug}`}
            />
            <SchemaOrg
                type="ItemList"
                data={{
            ...
            <div className="space-y-12">
                <nav className="flex items-center gap-4">
                    <Link href="/color-shades" className="p-2 hover:bg-white rounded-xl transition-colors text-gray-400 hover:text-gray-900 border border-transparent hover:border-gray-100 mb-4 inline-flex items-center gap-2 font-bold text-sm">
                ...
                <header className="flex flex-col md:flex-row gap-12 items-center md:items-start border-b border-gray-100 pb-16">
                    ...
                    <div className="space-y-6 text-center md:text-left">
                        <div className="space-y-2">
                            <h1 className="text-5xl md:text-7xl font-extrabold text-gray-900 leading-tight">Shades of {colorData.name}</h1>
                            <p className="font-mono text-2xl text-gray-400 font-bold uppercase">{colorData.hex}</p>
                        </div>
                        <p className="text-xl text-gray-600 leading-relaxed max-w-2xl">{colorData.description}</p>
                    </div>
                </header>
                ...
                <div className="space-y-24 pt-12">
                    <section className="space-y-8">
                        <div className="flex items-center gap-3">
                            ...
                            <h2 className="text-3xl font-bold text-gray-900">Tints <span className="text-gray-400 font-medium">(Lighter)</span></h2>
                        </div>
                        <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-10 gap-4">
                            {tints.map((color, index) => (
                                <BigColorCard key={`tint-${index}`} color={color} onClick={() => handleCopy(color)} copied={copiedColor === color} />
                            ))}
                        </div>
                    </section>
                    ...
                    <section className="space-y-8">
                        <div className="flex items-center gap-3">
                            <div className="w-1.5 h-8 bg-gray-900 rounded-full" />
                            <h2 className="text-3xl font-bold text-gray-900">Shades <span className="text-gray-400 font-medium">(Darker)</span></h2>
                        </div>
                        <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-10 gap-4">
                            {shades.map((color, index) => (
                                <BigColorCard key={`shade-${index}`} color={color} onClick={() => handleCopy(color)} copied={copiedColor === color} />
                            ))}
                        </div>
                    </section>
                </div>
                ...
                <div className="bg-blue-50/50 rounded-[2.5rem] p-10 flex flex-col md:flex-row items-center gap-8 border border-blue-100 mt-20">
                    ...
                    <div className="space-y-2 text-center md:text-left">
                        ...
                        <p className="text-blue-800 leading-relaxed">
                            Color tints are created by adding white to a pure color, while shades are created by adding black.
                            Using a consistent range of tints and shades helps create a balanced and professional design system.
                        </p>
                    </div>
                </div>
            </div>
        ...
        >
            ...
            >
                <div className="opacity-0 group-hover:opacity-100 transition-opacity">
                    {copied ? <Check size={24} className={isLight ? 'text-gray-900' : 'text-white'} /> : <Copy size={24} className={isLight ? 'text-gray-900/40' : 'text-white/40'} />}
                </div>
            </div>
            <div className="text-left space-y-1">
                <p className="font-mono text-[10px] font-bold text-gray-400 uppercase tracking-tighter group-hover:text-gray-900 transition-colors">{color}</p>
                <p className="text-[10px] font-bold text-gray-300 uppercase">{copied ? 'Copied' : 'Click to copy'}</p>
            </div>
        </motion.button>
    );
}
